// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.commands;

import edu.wpi.first.wpilibj.DoubleSolenoid.Value;
import edu.wpi.first.wpilibj2.command.CommandBase;
import frc.robot.subsystems.ArmSys;
import frc.robot.subsystems.Solenoids;

/**
* What do you think?
*/
public final class TelescoperOut extends CommandBase {
	boolean finished;
	public TelescoperOut(Solenoids inSysSolenoids, ArmSys inSysArm) {
		addRequirements(inSysSolenoids, inSysArm);
		finished = false;
	}

	@Override
	public void execute() {
//		  if ((sysArm.GetArmEnc() > 5500 ) || (sysArm.GetArmEnc() < 450 )
//		  &&
//		  RobotContainer.driverSecondController.getYButtonPressed()) {
//		   // sysSolenoids.ToggleTelescope(Value.kOff);
//		   finished = true;
//		  } else {
//		int armenc = ArmSys.armEncoder.get();
//		if ((armenc < 5500) || (armenc > 100)) {
//		sysSolenoids.ToggleTelescope(Value.kReverse);
//		}
//		   finished = true;
//		  }
//		if (sysSolenoids.GetTelescope() == Value.kReverse) {
//			finished = true;
//		} else {
//			finished = false;
//		}
		// FIXME: Fix limits on this. See above for the previous code. This probably won't work how it is.
		Solenoids.armTelescoper.set(Value.kReverse);
		finished = true;

	}

	@Override
	public void end(boolean interrupted) {
		Solenoids.armTelescoper.set(Value.kOff);
	}
	
	@Override
	public boolean isFinished() {
		return finished;
	}
}
